	...
	public static void initializeDB() throws FileNotFoundException, IOException {
		try {
			...
			String[] table_creation = new String[] {
					...
					"CREATE TABLE IF NOT EXISTS `mcms`.`class` (  `id` INT NOT NULL AUTO_INCREMENT,  `subject` VARCHAR(100) NOT NULL, `class_type` ENUM('individual', 'group') NOT NULL, `monthly_fee` DECIMAL NOT NULL,  `commence_date` DATE NOT NULL,  `max_students` INT NOT NULL,  `location` VARCHAR(255) NOT NULL,  PRIMARY KEY (`id`));",
					"CREATE TABLE IF NOT EXISTS `mcms`.`takes` (  `class_student_id` INT NOT NULL,  `class_id` INT NOT NULL,  `student_id` INT NOT NULL,  `enabled` TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'Whether the student is still studying (has not abandoned) the class. ',  PRIMARY KEY (`class_student_id`),  CONSTRAINT `takes_class`    FOREIGN KEY (`class_id`)    REFERENCES `mcms`.`class` (`id`)    ON DELETE NO ACTION    ON UPDATE NO ACTION,  CONSTRAINT `takes_student`    FOREIGN KEY (`student_id`)    REFERENCES `mcms`.`student` (`id`)    ON DELETE NO ACTION    ON UPDATE NO ACTION);",
					...
					"CREATE TABLE IF NOT EXISTS `mcms`.`date` (  `id` INT NOT NULL,  `class_id` INT NOT NULL,  `day` ENUM('Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat') NOT NULL,  `start_time` TIME NOT NULL,  `end_time` TIME NOT NULL,  PRIMARY KEY (`id`),  CONSTRAINT `class_id`    FOREIGN KEY (`class_id`)    REFERENCES `mcms`.`class` (`id`)    ON DELETE NO ACTION    ON UPDATE NO ACTION);"
