	...
	public static void initializeDB() throws FileNotFoundException, IOException {
		try {
			...
			String[] table_creation = new String[] {
					...
					"CREATE TABLE IF NOT EXISTS `mcms`.`session` (  `id` INT NOT NULL,  `class_id` INT NOT NULL, `teacher_id` INT NOT NULL,  `substitute_teacher` TINYINT(1) NOT NULL,  `date` DATE NOT NULL, PRIMARY KEY (`id`),  CONSTRAINT `session_class_id`    FOREIGN KEY (`class_id`)    REFERENCES `mcms`.`class` (`id`)    ON DELETE NO ACTION    ON UPDATE NO ACTION,  CONSTRAINT `session_teacher_id`    FOREIGN KEY (`teacher_id`)    REFERENCES `mcms`.`teacher` (`id`)    ON DELETE NO ACTION    ON UPDATE NO ACTION);",
