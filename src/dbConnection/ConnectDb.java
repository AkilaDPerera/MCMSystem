	...
	public static void initializeDB() throws FileNotFoundException, IOException {
		try {
			...
			String[] table_creation = new String[] {
					...
					"CREATE TABLE IF NOT EXISTS `mcms`.`user` (  `id` INT NOT NULL AUTO_INCREMENT,  `privileges` ENUM('admin', 'teacher') NOT NULL,  `username` VARCHAR(45) NOT NULL,  `password` VARCHAR(100) NOT NULL,  `email` VARCHAR(45) NULL, `teacher_id` INT,  PRIMARY KEY (`id`));",
					...
					"CREATE TABLE IF NOT EXISTS `mcms`.`session` (  `id` INT NOT NULL AUTO_INCREMENT,  `class_id` INT NOT NULL, `teacher_id` INT,  `substitute_teacher` TINYINT(1),  `date` DATE NOT NULL, PRIMARY KEY (`id`),  CONSTRAINT `session_class_id`    FOREIGN KEY (`class_id`)    REFERENCES `mcms`.`class` (`id`)    ON DELETE NO ACTION    ON UPDATE NO ACTION,  CONSTRAINT `session_teacher_id`    FOREIGN KEY (`teacher_id`)    REFERENCES `mcms`.`teacher` (`id`)    ON DELETE NO ACTION    ON UPDATE NO ACTION);",
					"CREATE TABLE IF NOT EXISTS `mcms`.`attendance` (  `session_id` INT NOT NULL,  `student_id` INT NOT NULL, `present` TINYINT(1),  CONSTRAINT `session_student`    FOREIGN KEY (`student_id`)    REFERENCES `mcms`.`student` (`id`)    ON DELETE NO ACTION    ON UPDATE NO ACTION,  CONSTRAINT `session_id`    FOREIGN KEY (`session_id`)    REFERENCES `mcms`.`session` (`id`)    ON DELETE NO ACTION    ON UPDATE NO ACTION);",
					"CREATE TABLE IF NOT EXISTS `mcms`.`teacher_attendance` (  `session_id` INT NOT NULL,  `teacher_id` INT NOT NULL, `present` TINYINT(1),  CONSTRAINT `session_teacher`    FOREIGN KEY (`teacher_id`)    REFERENCES `mcms`.`teacher` (`id`)    ON DELETE NO ACTION    ON UPDATE NO ACTION,  CONSTRAINT `session_teachers_id`    FOREIGN KEY (`session_id`)    REFERENCES `mcms`.`session` (`id`)    ON DELETE NO ACTION    ON UPDATE NO ACTION);"					
			};
			...
			String[] insert = new String[] {
					"insert into `mcms`.`user` values ('1', 'admin', 'demo','demo',' ', NULL);",
					"insert into `mcms`.`user` values ('2', 'teacher', 'demot','demot',' ', NULL);"
